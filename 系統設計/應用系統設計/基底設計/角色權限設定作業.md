...
3. **權限查詢來源**: 從 `role_rights` 表查詢使用者所屬角色的權限設定
...
開始
  ↓
選擇角色 (user_roles.is_active = true)
  ↓
查詢該角色的現有權限設定
  ↓
├─ 有設定 → 顯示現有權限供修改
└─ 無設定 → 顯示空白權限供新增
  ↓
載入所有已啟用功能 (system_functions.is_active = true)
  ↓
...
#### 2.2.1 角色選擇
- 下拉選單顯示所有啟用的角色 (`user_roles.is_active = true`)
...
1. **查詢現有權限**:
   ...
   WHERE user_roles_id = {selected_role_id}
   ```

2. **載入功能清單** (依角色類型過濾):
   ```sql
   -- 若選擇的角色為系統管理角色 (is_mana = true)
   SELECT * FROM system_functions
   WHERE is_active = true
   ORDER BY func_order ASC

   -- 若選擇的角色為非系統管理角色 (is_mana = false)
   SELECT * FROM system_functions
   WHERE is_active = true
     AND is_mana = false  -- 不顯示系統管理功能
   ORDER BY func_order ASC
   ```

   **功能過濾規則** (更新日期: 2026-01-19):
   - 系統管理角色 (`user_roles.is_mana = true`): 可設定所有功能的權限
   - 非系統管理角色 (`user_roles.is_mana = false`): 只能設定非系統管理功能 (`system_functions.is_mana = false`) 的權限
   - 目的: 防止非系統管理角色取得系統管理功能的權限

3. **建立權限矩陣**:
   - 橫軸: 功能清單 (依 func_order 排序,已過濾)
...
1. **新增權限 (Create)**:
   - 檢查: `system_functions.module_item` 是否包含 `"Create"`
   - 包含 → 啟用勾選框，可由使用者設定
   - 不包含 → 禁用勾選框，`is_create = false`

2. **讀取權限 (Read)**:
   - 檢查: `system_functions.module_item` 是否包含 `"Read"`
   - 包含 → 啟用勾選框，可由使用者設定
   - 不包含 → 禁用勾選框，`is_read = false`

3. **修改權限 (Update)**:
   - 檢查: `system_functions.module_item` 是否包含 `"Update"`
   - 包含 → 啟用勾選框，可由使用者設定
   - 不包含 → 禁用勾選框，`is_update = false`

4. **刪除權限 (Delete)**:
   - 檢查: `system_functions.module_item` 是否包含 `"Delete"`
   - 包含 → 啟用勾選框，可由使用者設定
   - 不包含 → 禁用勾選框，`is_delete = false`

5. **列印權限 (Print)**:
   - 檢查: `system_functions.module_item` 是否包含 `"Print"`
   - 包含 → 啟用勾選框，可由使用者設定
   - 不包含 → 禁用勾選框，`is_print = false`

6. **產檔權限 (File)**:
   - 檢查: `system_functions.module_item` 是否包含 `"File"`
   - 包含 → 啟用勾選框，可由使用者設定
...
```python
def save_role_rightss(user_roles_id: int, rights_data: List[RoleRightData]):
    """
    儲存角色權限設定
    ...
    try:
        ...
        db.query(RoleRight).filter(
            RoleRight.user_roles_id == user_roles_id
        ).delete()
        ...
        for right in rights_data:
            new_right = RoleRight(
                ...
                system_functions_id=right.system_functions_id,
...
CREATE TABLE IF NOT EXISTS role_rights (
    ...
    system_functions_id INTEGER NOT NULL,
    ...
    FOREIGN KEY (system_functions_id) REFERENCES system_functions(id) ON DELETE CASCADE,
    FOREIGN KEY (edit_by) REFERENCES users(id)
);

-- 索引
CREATE INDEX idx_role_rights_role ON role_rights(user_roles_id);
CREATE INDEX idx_role_rights_function ON role_rights(system_functions_id);
CREATE UNIQUE INDEX idx_role_rights_unique ON role_rights(user_roles_id, system_functions_id);

-- 註解
COMMENT ON TABLE role_rights IS '角色權限設定檔';
COMMENT ON COLUMN role_rights.id IS '資料序號';
COMMENT ON COLUMN role_rights.user_roles_id IS '角色編號';
COMMENT ON COLUMN role_rights.system_functions_id IS '功能編號';
COMMENT ON COLUMN role_rights.func_code IS '細部功能代碼';
COMMENT ON COLUMN role_rights.is_create IS '新增權限';
COMMENT ON COLUMN role_rights.is_read IS '讀取權限';
COMMENT ON COLUMN role_rights.is_update IS '修改權限';
COMMENT ON COLUMN role_rights.is_delete IS '刪除權限';
COMMENT ON COLUMN role_rights.is_print IS '列印權限';
COMMENT ON COLUMN role_rights.is_file IS '檔案權限';
COMMENT ON COLUMN role_rights.edit_by IS '編輯者';
COMMENT ON COLUMN role_rights.created_at IS '資料建立時間';
COMMENT ON COLUMN role_rights.updated_at IS '資料修改時間';
```
...
| user_roles_id | INTEGER | - | NOT NULL, FK | 角色編號 (參考 user_roles.id) |
| system_functions_id | INTEGER | - | NOT NULL, FK | 功能編號 (參考 system_functions.id) |
...
| edit_by | INTEGER | - | NOT NULL, FK | 編輯者 (參考 users.id) |
...
1. **主鍵約束**: `id` 為主鍵，自動遞增
2. **外鍵約束**:
   - `user_roles_id` → `user_roles(id)` - CASCADE DELETE
   - `system_functions_id` → `system_functions(id)` - CASCADE DELETE
   - `edit_by` → `users(id)`
3. **唯一性約束**: `(user_roles_id, system_functions_id)` 組合唯一，確保一個角色對一個功能只有一組權限設定
...
└────────┬─────────┘
         ...
         │
┌────────▼─────────┐         ┌──────────────────┐
│   role_rights     │  N:1    │   system_functions     │
│   (角色權限)      │─────────▶│   (系統功能)      │
│                  │         └──────────────────┘
│   - is_create    │
│   - is_read      │
...
#### 4.1.2 取得功能清單與可用權限
```
GET /api/role_rights/functions?role_id={role_id}
```

**說明**: 取得所有已啟用功能及其支援的權限項目

**查詢參數**:
- `role_id` (integer, optional): 角色ID - 用於過濾功能清單
  - 若提供且角色為非系統管理角色 (`is_mana = false`),則過濾掉系統管理功能 (`is_mana = true`)
  - 若未提供或角色為系統管理角色,則顯示所有已啟用功能
...
#### 4.1.4 刪除角色權限設定
```
DELETE /api/role_rights/{role_id}
```
...
class RoleRight(Base):
    ...
    # 關聯欄位
    user_roles_id = Column(Integer, ForeignKey("user_roles.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    # 索引與約束
    __table_args__ = (
        Index("idx_role_rights_role", "user_roles_id"),
        Index("idx_role_rights_function", "system_functions_id"),
        Index("idx_role_rights_unique", "user_roles_id", "system_functions_id", unique=True),
    )

    # 關聯
    role = relationship("UserRole", foreign_keys=[user_roles_id])
    function = relationship("SysFunction", foreign_keys=[system_functions_id])
    editor = relationship("UserDetail", foreign_keys=[edit_by])
```

### 5.2 資料驗證 (Schema)

**檔案**: `app/schemas/role_rights.py`
...
class RoleRightBase(BaseModel):
    """角色權限基本資料"""
    system_functions_id: int = Field(..., description="功能編號")
...
async def get_role_rightss(
    ...
    # 查詢權限設定
    rights = db.query(RoleRight).filter(RoleRight.user_roles_id == role_id).all()
...
async def save_role_rightss(
    ...
    try:
        ...
        deleted_count = db.query(RoleRight).filter(
            RoleRight.user_roles_id == role_id
        ).delete()
        ...
        for right in data.rights:
            # 驗證功能是否存在
            function = db.query(SysFunction).filter(SysFunction.id == right.system_functions_id).first()
            if not function:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
                    detail=f"找不到功能ID: {right.system_functions_id}"
                )
...
```python
from app.routes import role_rights
...
**檔案**: `src/services/roleRightsService.ts`
...
**檔案**: `src/pages/RoleRightsPage.tsx`
...
const RoleRightPage: React.FC = () => {
  ...
  const handleRoleChange = async (roleId: number) => {
    ...
    try {
      ...
      data.rights.forEach(right => {
        rightsMap.set(right.system_functions_id, right);
      });
  ...
  const handlePermissionChange = (
    funcId: number,
    funcCode: string,
    permission: keyof Omit<RoleRight, 'system_functions_id' | 'func_code' | 'id'>,
    value: boolean
  ) => {
    const newRights = new Map(rights);
    const right = newRights.get(funcId) || {
      system_functions_id: funcId,
      func_code: funcCode,
      is_create: false,
      is_read: false,
      is_update: false,
...
**檔案**: `src/styles/RoleRightsPage.css`
...
def check_permission(
    ...
    """
    # 取得使用者的所有角色ID
    role_ids: List[int] = user.user_roles if isinstance(user.user_roles, list) else []
    ...
    rights = db.query(RoleRight).filter(
        RoleRight.user_roles_id.in_(role_ids),
...
- **唯一性約束**: `(user_roles_id, system_functions_id)` 組合唯一，防止重複設定
...
| 1.0.1 | 2026-01-19 | 修正 API 文件:新增 role_id 參數說明、更新程式碼範例以反映實際實現 | Claude |
| 1.0.2 | 2026-01-23 | 正名化更新：資料表與API路徑更新為複數形式 | Claude |
| 1.0.2 | 2026-01-23 | 正名化更新：資料表與API路徑更新為複數形式 | Claude |
...
```bash
# 取得功能清單
curl -X GET "http://localhost:10181/api/role_rights/functions" \
  -H "Authorization: Bearer {token}"

# 取得角色權限
curl -X GET "http://localhost:10181/api/role_rights/1" \
  -H "Authorization: Bearer {token}"

# 儲存權限設定
curl -X POST "http://localhost:10181/api/role_rights/1" \
  -H "Authorization: Bearer {token}" \
