...
2. **載入功能清單** (依角色類型過濾):
   ...
   **功能過濾規則** (更新日期: 2026-01-19):
   - 系統管理角色 (`user_role.is_mana = true`): 可設定所有功能的權限
   - 非系統管理角色 (`user_role.is_mana = false`): 只能設定非系統管理功能 (`sysfuction.is_mana = false`) 的權限
...
#### 4.1.2 取得功能清單與可用權限
```
GET /api/role_right/functions?role_id={role_id}
```
